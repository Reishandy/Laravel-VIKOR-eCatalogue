...
export default function CyberProductCard({
    ...
    return (
        <div
            ...
            className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-sm border border-space-border bg-space-900/40 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-space-accent/60 hover:shadow-[0_0_30px_rgba(167,139,250,0.1)]"
        >
            {/* Image Area */}
            <div className="relative h-56 w-full overflow-hidden border-b border-space-border bg-space-950">
                {item.rank && (
                    <div className="absolute top-3 left-3 z-10">
                        <div className="flex items-center gap-1.5 rounded-md border border-space-highlight/40 bg-space-900/80 px-2.5 py-1.5 backdrop-blur-sm transition-all duration-300 hover:border-space-highlight/60 hover:bg-space-900/90">
                ...
                {/* Glow effect */}
                <div className="absolute inset-0 bg-gradient-to-t from-space-900 to-transparent opacity-80" />

                {item.image ? (
                    <img
                        src={item.image}
                        alt={item.name}
                        className="h-full w-full scale-100 object-cover opacity-70 contrast-125 grayscale transition-all duration-500 group-hover:scale-105 group-hover:opacity-100 group-hover:grayscale-0"
                    />
                ) : (
            ...
            <div className="flex flex-1 flex-col p-6">
                ...
                <div className="mb-4 flex items-baseline gap-2">
                    ...
                    <span className="font-mono text-sm text-space-muted uppercase">
                        Unit Price
                    </span>
                </div>

                <p className="mb-6 line-clamp-3 flex-grow text-xs leading-relaxed font-light text-space-muted">
                ...
                <div className="mt-auto flex items-center justify-between border-t border-dashed border-space-border pt-4">
                    <div className="flex items-center gap-2 font-mono text-sm text-space-muted">
